...
export const thanhToan = async (req, res) => {
  try {
    ...
    if (donDatVe.trang_thai === "đã xác nhận") {
    ...
    await DonDatVe.findByIdAndUpdate(idDonDatVe, { trang_thai: "đã xác nhận" });

    res.status(200).json({ message: "Thanh toán thành công! Vé của bạn đã được xác nhận." });
